...
export class LspDocument {
    ...
    offsetAt(position: lsp.Position): number {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (position.line === i) {
                if (l.length < position.character) {
                    throw new Error(`Position ${JSON.stringify(position)} is ` +
                                    `out of range. Line [${i}] only has ` +
                                    `length ${l.length}.`);
                }
        ...
        throw new Error(`Position ${JSON.stringify(position)} is out of ` +
                        `range. Document only has ${lines.length} lines.`);
    }

    apply(contentChanges: lsp.TextDocumentContentChangeEvent[], version: number) {
        this.applyEdits(contentChanges.map(e => {
            const range = e.range || lsp.Range.create(lsp.Position.create(0, 0),
